...
lower= np.array([12,84,95],np.uint8) # Array (H,S,V) for the lower threshold bound of the HSV image
upper= np.array([38,255,255],np.uint8) # Array (H,S,V) for the upper threshold bound of the HSV image
...
width = 410                         # Width of the image, this is sent in our point message as the z-component to know the zero point in the frame.
fov = 62.2
...
def get_image(CompressedImage):
    ...
    red_mask1 = cv2.inRange(imgHSV, lower_red, upper_red)
    # Color Mask
    lower_red = np.array([0, 80, 50])
    upper_red = np.array([10, 255, 200])
    # Make a mask to show only allowable color from image
    red_mask2 = cv2.inRange(imgHSV, lower_red, upper_red)
    ...
    if centers:
        ...
        objectAngle = (fov/float(width))*((float(width)/2.0)-p.x)
    else:
...
def Init():
    ...
    #Create a publisher that will be publishing Geometric message Points
    global pubtest
    pubtest = rospy.Publisher('angle', Float32, queue_size=10)
