...
class Drive_Wheels():
    def __init__(self):
        ...
        self.command = 0
        self.twist = Twist()
        self.turn_coord = self.globalPos
        self.drive_wheels()
    ...
    def drive_to_sign(self):
        if self.wall_dist != -1 and self.wall_dist != 0:
            if self.wall_dist > 0.6:
                self.twist.linear.x = 0.1
                if -0.1 < self.desired_angle < 0.1:
                    coord_error = self.turn_coord.y - self.globalPos.y
                elif (math.pi / 2 - 0.1) < self.desired_angle < (math.pi / 2 + 0.1):
                    coord_error = self.globalPos.x - self.turn_coord.x
                elif (-math.pi / 2 - 0.1) < self.desired_angle < (-math.pi / 2 + 0.1):
                    coord_error = self.turn_coord.x - self.globalPos.x
                else:
                    coord_error = self.globalPos.y - self.turn_coord.y
                self.twist.angular.z = 0.1*coord_error
